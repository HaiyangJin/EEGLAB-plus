%% this file outputs all ERP data for different electrode, condition and 
...
if ispc || ismac
    ...
    save(fileNameBackup, 'table_MeanRaw', 'cell_MeanRawIVs', 'cell_MeanRawDVs', 'expFolder', '-v7.3') %, '-nocompression'     
elseif isunix
    save(fileNameBackup, '-v7.3') %, '-nocompression'     
else
    disp('Platform not supported')
end
...
if ispc || ismac
    ...
    save(fileNameBackup, 'table_PeakValues', 'cell_PeakValueIVs', 'cell_PeakValueDVs', '-v7.3') %, '-nocompression'
elseif isunix
    save(fileNameBackup, '-v7.3') %, '-nocompression'
else
    disp('Platform not supported')
