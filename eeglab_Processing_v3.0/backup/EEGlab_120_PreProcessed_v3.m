...
for iFile = 1:nFiles
    ...
    elseif basedAcc == 2
        ...
        if strcmp(experimentNum, '3') || strcmp(experimentNum, '4')
            events = horzcat(events, {'NF7+_RES0' 'NH7+_RES0'}); % add the labels for incorrect 17ms
        end
        
        if strcmp(experimentNum, '2') || strcmp(experimentNum, '3')  % add the lables for scramble trials
            events = horzcat(events, {'SF7+'  'SF5+'  'SF1+'  'SF2+'  'SH7+'  'SH5+'  'SH1+'  'SH2+'});
        elseif strcmp(experimentNum, '4')
            events = horzcat(events, {'SF7+'  'SF2+'  'SH7+'  'SH2+'});
        end
    ...
    elseif basedAcc == 3
        ...
        tempEventLogical4 = arrayfun(@(x) strcmp(x{:}(9),'1'), tempNormalEvents); % labels for scrambled stimuli with ACC only
        normalEvents = tempEvents(tempEventLogical4);
        
        
        switch experimentNum
            case '1'
                error('Accurate data could not be used with EEG data!');
            case '2'
